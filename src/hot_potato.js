...
module.exports = {
    startPotato: async function startPotato(client, interaction) {
        // get all users in the guild and filter out the bots
        ...
        //setInterval(this.kickPlayer, 10000, client, interaction);
    },
    PassPotato: async function PassPotato(client, interaction, targetUser) {
        ...
        db.get("SELECT HasPotato FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            ...
            if(interaction.user.username != player_with_potato){
                interaction.editReply({content: "You do not have the potato. " + player_with_potato + " has the potato.", ephemeral: true, });
                return;
            } else {
                db.get("SELECT RemainingPlayers FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
                    ...
                    if (!targetUser) {
                        ...
                        interaction.editReply({content: "You can't pass the hot potato to yourself", ephemeral: true,});
                        return;
                    } else if (targetUser.user.bot) {
                        interaction.editReply({content: "You can't pass the hot potato to a bot", ephemeral: true,});
                        return;
                    } else if (!check) {
                        interaction.editReply({content: "This user is currently not in the game", ephemeral: true,});
                        return;
                    } else {
                        ...
                        interaction.editReply("The hot potato has been passed to " + targetUser.user.username);
                    }
                });
            }
        });
        ...
        db.get("SELECT HasPotato FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            ...
            if(interaction.user.username != player_with_potato){
                ...
                db.get("SELECT RemainingPlayers FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
                    ...
                    interaction.editReply("The hot potato has been passed to " + new_player_with_potato);
                });
            }
        });
    },
    getPotatoHolder: async function getPotatoHolder(client, interaction){
        const correct_guild = await interaction.guild.id;
        db.get("SELECT HasPotato FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            interaction.editReply(`${column.HasPotato} has the potato`);
        });
    },
    gameEnded: async function gameEnded(client, interaction){
        ...
        db.get("SELECT RemainingPlayers FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            ...
            interaction.editReply(`<@${userID}> is the winner!`);
        });
    ...
    kickPlayer: async function kickPlayer(client, interaction){
        ...
        db.get("SELECT RemainingPlayers, HasPotato FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            ...
            if(current_players_arr.length == 0){
                ...
                interaction.editReply(player_with_potato + " is out. " + new_player_with_potato + " has the potato.");
            }
        });
    },
    getRemainingPlayers: async function getRemainingPlayers(client, interaction){
        ...
        db.get("SELECT RemainingPlayers FROM Game WHERE GuildID = " + correct_guild, (err, column)=>{
            ...
            interaction.editReply("Players remaining: " + current_players);
        });
    },
    ...
    /*checkPotato: function checkPotato(client, guildId){
